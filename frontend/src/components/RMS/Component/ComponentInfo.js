...
const ComponentInfo = ({activeStep, updateStep}) => {
    ...
    useEffect(() => {
        ...
        fetchData('https://power.bpm.cit.tum.de/henryk_backend/rms_service/language_models/', setLLMs)
        // Fetch Tasks from: list of strings: format: [task1, task2, ...]
        fetchData('https://power.bpm.cit.tum.de/henryk_backend/rms_service/data/tasks/', setTasks);
        // Fetch Types from: list of strings: format: [type1, type2, ...]
        fetchData('https://power.bpm.cit.tum.de/henryk_backend/rms_service/data/types/', setTypes);
