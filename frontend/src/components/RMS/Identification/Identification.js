...
const Identification = ({activeStep, updateStep}) => {
    ...
    useEffect(() => {
        ...
        fetchData('http://x.x.x.x:4999/rms_service/domains/', setDomains);
        // Fetch Tasks from: list of strings: format: [task1, task2, ...]
        fetchData('http://x.x.x.x:4999/rms_service/purposes/', setPurposes);
        // Fetch LLMs from: list of json (dict): format: [{_id: string, name: string, size: string}]
        fetchData('http://x.x.x.x:4999/rms_service/capabilities/', setCapabilities);
        // Fetch Domain from: list of json (dict): format: [{_id: string, name: string}]
        fetchData('http://x.x.x.x:4999/rms_service/llm_users/', setLLMUsers);
        // Fetch Types from: list of strigns: format: [type1, type2, ...]
        fetchData('http://x.x.x.x:4999/rms_service/llm_subjects/', setLLMSubjects);
