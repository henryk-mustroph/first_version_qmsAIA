...
const Analysis = ({activeStep, updateStep}) => {
    ...
    useEffect(() => {
        ...
        fetchData('http://x.x.x.x:4999/rms_service/metrics/type/Performance/', setPerformanceMetrics);
        // Fetch all consistency metrics
        fetchData('http://x.x.x.x:4999/rms_service/metrics/type/Consistency/', setConsistencyMetrics);
        // Fetch all explainability metrics
        fetchData('http://x.x.x.x:4999/rms_service/metrics/type/Explainability/', setExplainabilityMetrics);
    ...
    const createRiskIdentification = async (language_model, domain, purpose, capability, llm_user, llm_subject) => {
        ...
        try {
            // Production
            const response = await fetch('http://x.x.x.x:4999/rms_service/risk_identifications/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const getDataByDomainAndType = async (domain_id, type) => {
        try {
            // Production
            const response = await fetch(`http://x.x.x.x:4999/rms_service/data/${domain_id}/${type}/`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    const createRiskAnalysis = async (language_model, data, metrics) => {
        ...
        try {
            ...
            const response = await fetchWithTimeout('http://x.x.x.x:4999/rms_service/risk_analyses/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const createRiskAssessment = async (user_id, timestamp, risk_identification, risk_analysis) => {
        ...
        try {
            // Production
            const response = await fetch('http://x.x.x.x:4999/rms_service/risk_assessments/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            if (riskIdentification != null && riskIdentification._id != null) {
                try {
                    // Production
                    const deleteMsg = deleteRiskProcessSubsteps(`http://x.x.x.x:4999/rms_service/risk_identifications/${riskIdentification._id}/`);
            ...
            if (riskAnalysis != null && riskAnalysis._id != null) {
                try {
                    // Production
                    const deleteMsg = deleteRiskProcessSubsteps(`http://x.x.x.x:4999/rms_service/risk_analyses/${riskAnalysis._id}/`);
                    console.log(deleteMsg);
                } catch (error) {
                    // Handle errors here
                    console.error('Error:', error);
