...
const DataPage = () => {
    ...
    useEffect(() => {
        ...
        fetchData('http://x.x.x.x:4999/dmdgs_service/domains/', setDomains);
        fetchData('http://x.x.x.x:4999/dmdgs_service/types/', setTypes);
        fetchData('http://x.x.x.x:4999/dmdgs_service/language_models/', setLlms);
    }, []);
    ...
    const createTrainingDataCheck = async (description) => {
        ...
        try {
            ...
            //Production
            const response = await fetch('http://x.x.x.x:4999/dmdgs_service/training_data_checks/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const createTrainingData = async (name, origin, type, domain, size, llms, training_data_check) => {
        ...
        try {
            // Production
            const response = await fetch('http://x.x.x.x:4999/dmdgs_service/training_data/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const updateUserAddTrainingData = async (user_id, trainingData_id) => {
        try {
            // Production
            const response = await fetch(`http://x.x.x.x:4999/dmdgs_service/users/${user_id}/`, {
                method: 'PUT',
                headers: {
